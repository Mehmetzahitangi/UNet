...
def double_conv(in_channel_size, out_channel_size):
    conv = nn.Sequential(
        nn.Conv2d(in_channels = in_channel_size, out_channels = out_channel_size, kernel_size = 3, padding = 1),
        nn.ReLU(inplace = True),
        nn.Conv2d(in_channels = out_channel_size, out_channels = out_channel_size, kernel_size = 3, padding = 1),
        nn.ReLU(inplace = True)
    )
...
class UNet(nn.Module):
    def __init__(self,num_of_classes):
        ...
        self.output = nn.Conv2d(in_channels = 64, out_channels = num_of_classes, kernel_size=3, padding=1) 
    ...
    def forward(self,image):
        ...
        layer11 = torch.cat((layer10,layer7),dim=1)
        ...
        output_layer = func.log_softmax(output,1)

        return output_layer

model = UNet(3)

aug_data = torch.rand(size=(1,1,128,256))
